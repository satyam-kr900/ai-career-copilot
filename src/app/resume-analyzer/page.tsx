...
import { FileText, CheckCircle2, User, Mail, Phone, MapPin, Globe, Github, Linkedin } from 'lucide-react';
...
export default function ResumeAnalyzerPage() {
  ...
  return (
    <div className="space-y-6 pb-12">
      <Header title="Advanced Resume Parser & ATS Analyzer" />
            ...
            {parsedResume ? (
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6">
                <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                  <div>
                    <h3 className="text-xl font-bold text-slate-100">{parsedResume.name}</h3>
                    <p className="text-xs text-slate-400 mt-0.5">{parsedResume.summary}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-2xl font-black text-indigo-400">{parsedResume.completenessScore}%</span>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center text-slate-400 space-y-3">
                ...
                <h4 className="font-bold text-slate-200">No Resume Parsed Yet</h4>
                <p className="text-xs">Upload a PDF or DOCX resume to test 18+ section automatic field extraction.</p>
              </div>
            )}
