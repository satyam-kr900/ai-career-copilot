...
export default function JobMatcherPage() {
  ...
  return (
    <div className="space-y-6 pb-12">
      <Header title="Semantic Job Matching & Recommendation Engine" />

      <div className="px-6 space-y-6">
        <div className="bg-gradient-to-r from-indigo-950/60 via-slate-900 to-slate-900 border border-indigo-500/30 rounded-2xl p-6 space-y-2">
          <h3 className="font-bold text-white text-base flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-indigo-400" /> Vector Cosine Semantic Matching
          </h3>
        ...
        <div className="space-y-4">
          {jobs.map((job, idx) => (
            <div key={idx} className="bg-slate-900 border border-slate-800 hover:border-indigo-500/50 rounded-2xl p-6 space-y-4 transition-all">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="px-4 py-2 rounded-xl bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 font-black text-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div>
                  <span className="font-bold text-emerald-400 flex items-center gap-1 mb-2">
                    <CheckCircle2 className="w-4 h-4" /> Matching Candidate Skills:
                  </span>
                ...
                <div>
                  <span className="font-bold text-amber-400 flex items-center gap-1 mb-2">
                    <AlertCircle className="w-4 h-4" /> Missing / Learning Recommended:
                  </span>
                  <div className="flex flex-wrap gap-1.5">
                    {job.missingSkills.map((s, i) => (
                      <span key={i} className="px-2.5 py-1 rounded-md bg-amber-500/10 text-amber-300 border border-amber-500/20 font-medium">
                        {s}
                      </span>
                    ))}
                  </div>
              ...
              <div className="bg-slate-850 rounded-xl p-3 text-xs text-slate-300 border border-slate-800">
