...
// Section anchors used in "Why This Score" to link to specific resume areas
...
export default function ATSScoreCard({ ats }: { ats: ATSScoreBreakdown }) {
  const getScoreColor = (score: number) => {
    // WCAG AA-compliant colours on dark (slate-900) background
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl">
        <div className="flex items-center gap-6">
          <div
            className={`w-24 h-24 rounded-2xl border-2 flex flex-col items-center justify-center shadow-lg ${getScoreColor(ats.overallScore)}`}
            role="img"
        ...
        <div className="grid grid-cols-2 gap-3 w-full md:w-auto">
          {[
            ...
            <div key={label} className="bg-slate-800/80 border border-slate-700/60 rounded-xl p-3 text-center">
            ...
            <div key={idx} className="space-y-1">
              <div className="flex justify-between text-xs font-medium">
                <a
                  ...
                  className="text-slate-300 hover:text-indigo-300 underline-offset-2 hover:underline transition"
                >
                  {item.label}
                  <span className="text-slate-500 ml-1">({item.weight} weight)</span>
                </a>
                <span className={`font-bold ${item.score >= 80 ? 'text-emerald-300' : item.score >= 65 ? 'text-amber-300' : 'text-rose-300'}`}>
                  {item.score}%
                </span>
              </div>
      ...
      {/* ── Why This Score — linked to resume sections ────────────── */}
      <div className="bg-indigo-950/30 border border-indigo-500/20 rounded-2xl p-5 text-xs space-y-3">
